...
def apply_specaugment(spec, max_time_mask=15, max_freq_mask=10):
    spec_aug = spec.copy()
    f_len = spec_aug.shape[0]
    f_width = random.randint(0, max_freq_mask)
    f0 = random.randint(0, max(1, f_len - f_width))
    spec_aug[f0:f0+f_width, :, :] = 0.0
    
    t_len = spec_aug.shape[1]
    t_width = random.randint(0, max_time_mask)
    t0 = random.randint(0, max(1, t_len - t_width))
    spec_aug[:, t0:t0+t_width, :] = 0.0
    return spec_aug
...
def extract_pcen_spec(y):
    ...
    mel = librosa.feature.melspectrogram(
        y=y, sr=SR, n_fft=N_FFT, hop_length=HOP, n_mels=N_MELS, fmin=300, fmax=8000
    )
    ...
    return pcen.T[..., np.newaxis].astype(np.float32)

def audio_generator(files, labels, is_training=True, label_smoothing=0.05):
        ...
        try:
            ...
            for idx in selected_indices:
                ...
                if is_training and random.random() < 0.5:
                    spec = apply_specaugment(spec)
                    
                label_vec = np.full((NUM_CLASSES,), label_smoothing / (NUM_CLASSES - 1), dtype=np.float32)
                label_vec[l] = 1.0 - label_smoothing
...
val_ds = tf.data.Dataset.from_generator(
    lambda: audio_generator(val_files, val_labels, is_training=False, label_smoothing=0.0),
    output_signature=(
        tf.TensorSpec(shape=INPUT_SHAPE, dtype=tf.float32),
        tf.TensorSpec(shape=(NUM_CLASSES,), dtype=tf.float32)
    )
...
def build_compact_cnn_opt(input_shape=(128, 128, 1), num_classes=27):
    ...
    model = keras.Model(inputs, outputs, name="Compact_CNN_27Sp_Opt")
    return model

model = build_compact_cnn_opt(input_shape=INPUT_SHAPE, num_classes=NUM_CLASSES)

epochs = 40
steps_per_epoch = max(1, len(train_files) * 3 // 16)
...
MODEL_SAVE_PATH = os.path.join(DATASET_DIR, 'best_model_compact_27sp_opt.keras')
...
print("\n--- Lancement de l'Entraînement Compact CNN 27-Espèces OPTIMISÉ ---")
...
print(f"\n==========================================")
print(f"Meilleure Précision de Validation Compact CNN 27-Espèces Optimisé: {val_acc*100:.2f}%")
print(f"==========================================")
...
plt.title('Compact CNN 27-Espèces Opt Accuracy')
plt.legend()

plt.subplot(1, 2, 2)
plt.plot(history.history['loss'], label='Train Loss')
plt.plot(history.history['val_loss'], label='Val Loss')
plt.title('Compact CNN 27-Espèces Opt Loss')
plt.legend()

plot_path = os.path.join(DATASET_DIR, 'compact_27sp_opt_history.png')
