...
# Readable species names for presentation
species_display_names = [name.replace('_', ' ').title() for name in safe_labels]

print("Loading 3 Models for Soutenance Benchmark Charts...")
...
print(f"Generating predictions on {len(val_files_64)} validation files...")
...
for f64, f128, true_label in zip(val_files_64, val_files_128, val_labels):
    try:
        ...
        tri_seg_preds = []
        for s64, s128 in zip(specs_64, specs_128):
            p_myna = myna(s64[np.newaxis, ..., np.newaxis].astype(np.float32), training=False).numpy()[0]
            p_compact = compact(s128.T[np.newaxis, ..., np.newaxis].astype(np.float32), training=False).numpy()[0]
            p_effnet = effnet(s128.T[np.newaxis, ..., np.newaxis].astype(np.float32), training=False).numpy()[0]
            
            # Optimal Tri-Ensemble weights
            tri_p = 0.35 * p_myna + 0.40 * p_compact + 0.25 * p_effnet
            tri_seg_preds.append(tri_p)
            
        avg_tri = np.max(tri_seg_preds, axis=0)
        pred_label = np.argmax(avg_tri)
...
print(f"\nTri-Ensemble Accuracy: {acc:.2f}%")

# 1. Generate 27x27 Confusion Matrix
...
plt.close()
print(f"Successfully generated Confusion Matrix at {cm_path}")

# 2. Generate Per-Class F1-Scores Chart
...
plt.close()
print(f"Successfully generated F1-Score Chart at {f1_path}")
...
plt.close()
print(f"Successfully generated Project Evolution Chart at {evo_path}")

print("\nAll presentation-ready charts successfully generated in dataset/soutenance_charts/!")
