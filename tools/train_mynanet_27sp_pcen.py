        ...
        try:
            ...
            for idx in selected_indices:
                ...
                mel = librosa.feature.melspectrogram(
                    y=chunk, sr=SR, n_fft=N_FFT, win_length=400, hop_length=HOP, n_mels=N_MELS, fmin=300, fmax=8000
                )
                ...
                spec = pcen[..., np.newaxis].astype(np.float32)
...
def build_mynanet_pcen(input_shape=(64, 300, 1), num_classes=27):
    ...
    x = layers.MaxPooling2D((2, 2))(x) # (32, 150)
    ...
    x = layers.Activation('swish')(x)
    x = se_block(x)
    x = layers.MaxPooling2D((2, 2))(x) # (16, 75)
    ...
    x = layers.Activation('swish')(x)
    x = se_block(x)
    x = layers.MaxPooling2D((2, 2))(x) # (8, 37)
    ...
    model = keras.Model(inputs, outputs, name="MynaNet_27Sp_PCEN")
    return model

model = build_mynanet_pcen(input_shape=INPUT_SHAPE, num_classes=NUM_CLASSES)
...
MODEL_SAVE_PATH = os.path.join(DATASET_DIR, 'best_model_mynanet_27sp_pcen.keras')
...
print("\n--- Lancement de l'Entraînement MynaNet 27-Espèces PCEN (Balanced Weights) ---")
...
print(f"\n==========================================")
print(f"Meilleure Précision de Validation MynaNet 27-Espèces PCEN: {val_acc*100:.2f}%")
print(f"==========================================")
...
plt.title('MynaNet 27-Espèces PCEN Accuracy')
plt.legend()

plt.subplot(1, 2, 2)
plt.plot(history.history['loss'], label='Train Loss')
plt.plot(history.history['val_loss'], label='Val Loss')
plt.title('MynaNet 27-Espèces PCEN Loss')
plt.legend()

plot_path = os.path.join(DATASET_DIR, 'mynanet_27sp_pcen_history.png')
