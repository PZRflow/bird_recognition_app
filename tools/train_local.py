...
checkpoint = ModelCheckpoint(os.path.join(DATASET_DIR, 'best_model.keras'), save_best_only=True, monitor="val_accuracy")
...
model.save(os.path.join(DATASET_DIR, 'saved_model.keras'))

# Plot accuracy
plt.figure(figsize=(10, 6))
plt.plot(history.history['accuracy'], label='Train Accuracy')
plt.plot(history.history['val_accuracy'], label='Validation Accuracy')
plt.title('Courbe d\'apprentissage - Compact CNN avec Sigmoid + BCE')
plt.ylabel('Accuracy')
plt.xlabel('Epoch')
plt.legend()
plt.savefig(os.path.join(DATASET_DIR, 'training_history.png'))
print("Modèle Compact CNN sauvegardé.")
