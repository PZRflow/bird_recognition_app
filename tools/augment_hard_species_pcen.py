...
def extract_audio_segments_pcen(wav_path):
    try:
        ...
        # Original
        pcen_64_orig = [extract_pcen(c, n_fft=1024, hop=160, n_mels=64, time_frames=300) for c in top_chunks]
        pcen_128_orig = [extract_pcen(c, n_fft=512, hop=375, n_mels=128, time_frames=128) for c in top_chunks]
...
def generate_augmented_tensors():
    print("Generating Targeted Pitch-Shifted (.npy) PCEN Tensors for 5 Hard Species...")
...
def cleanup_augmented_tensors():
    print("Cleaning up and deleting all augmented tensor files (*_aug*.npy)...")
    aug_files = glob.glob(os.path.join(CACHE_DIR, 'train', '*', '*_aug*.npy'))
    for f in aug_files:
        try:
            os.remove(f)
        except Exception:
            pass
    print(f"Cleaned up {len(aug_files)} augmented files successfully!")
