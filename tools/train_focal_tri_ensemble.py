...
HARD_SPECIES = [
    "blue_eared_barbet",
    "pin_striped_tit_babbler",
    "plaintive_cuckoo",
    "puff_throated_babbler",
    "pygmy_cupwing"
]

hard_indices = [i for i, name in enumerate(safe_labels) if name in HARD_SPECIES]

print(f"Targeting 5 Hard Species with Focal Weight Boost (1.6x):")
for idx in hard_indices:
    print(f"  - [{idx}] {safe_labels[idx]}")

NUM_CLASSES = len(safe_labels)
INPUT_SHAPE_64 = (64, 300, 1)

def get_cached_dataset_files(split, suffix='_64.npy', max_per_class=800):
    all_files_by_class = []
...
train_files_64, train_labels_64 = get_cached_dataset_files('train', '_64.npy', max_per_class=800)
val_files_64, val_labels_64 = get_cached_dataset_files('val', '_64.npy', max_per_class=400)

# Calculate base balanced class weights
class_weights_arr = compute_class_weight('balanced', classes=np.unique(train_labels_64), y=train_labels_64)

# Apply 1.6x Focal Multiplier to the 5 hard species
for idx in hard_indices:
    class_weights_arr[idx] *= 1.60

class_weights_dict = dict(enumerate(class_weights_arr))
print("Successfully calculated Focal Class Weights!")
...
def build_mynanet_focal(input_shape=(64, 300, 1), num_classes=27):
    ...
    model = keras.Model(inputs, outputs, name="MynaNet_27Sp_Focal")
    return model

model = build_mynanet_focal(input_shape=INPUT_SHAPE_64, num_classes=NUM_CLASSES)
...
MODEL_SAVE_PATH = os.path.join(DATASET_DIR, 'best_model_mynanet_27sp_focal.keras')
...
print("\n--- Lancement de l'Entraînement MynaNet FOCAL (Spécial 5 Espèces Difficiles) ---")
...
print(f"\n==========================================")
print(f"Meilleure Précision MynaNet Focal (5 Espèces Ciblées): {val_acc*100:.2f}%")
print(f"==========================================")
