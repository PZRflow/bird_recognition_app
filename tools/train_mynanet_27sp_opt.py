...
def apply_specaugment(spec, max_time_mask=20, max_freq_mask=8):
    spec_aug = spec.copy()
    # Frequency masking
...
def build_mynanet_opt(input_shape=(64, 300, 1), num_classes=27):
    ...
    model = keras.Model(inputs, outputs, name="MynaNet_27Sp_Opt")
    return model

model = build_mynanet_opt(input_shape=INPUT_SHAPE, num_classes=NUM_CLASSES)
...
MODEL_SAVE_PATH = os.path.join(DATASET_DIR, 'best_model_mynanet_27sp_opt.keras')
...
print("\n--- Lancement de l'Entraînement MynaNet 27-Espèces OPTIMISÉ (SpecAugment + Label Smoothing + Cosine LR) ---")
...
print(f"\n==========================================")
print(f"Meilleure Précision de Validation MynaNet 27-Espèces Optimisé: {val_acc*100:.2f}%")
print(f"==========================================")
...
plt.title('MynaNet 27-Espèces Opt Accuracy')
plt.legend()

plt.subplot(1, 2, 2)
plt.plot(history.history['loss'], label='Train Loss')
plt.plot(history.history['val_loss'], label='Val Loss')
plt.title('MynaNet 27-Espèces Opt Loss')
plt.legend()

plot_path = os.path.join(DATASET_DIR, 'mynanet_27sp_opt_history.png')
