...
# Audio parameters (PCEN + Spectral Equalization MynaNet)
SR = 16000
N_FFT = 1024
HOP = 160
N_MELS = 64
...
def audio_generator(files, labels):
    for f, l in zip(files, labels):
        try:
            ...
            energy_threshold = max_energy * 0.1 # 10% threshold
            
            selected_indices = []
            for energy, idx in chunks_energy:
                if len(selected_indices) >= 5: # Max 5 segments
                    break
            ...
            for idx in selected_indices:
                ...
                # 2. Spectral Noise Equalization (Background noise subtraction)
...
def tf_spec_augment(mel, label):
    ...
    for _ in range(num_mel_mask):
        f = tf.random.uniform([], minval=0, maxval=8, dtype=tf.int32)
        f0 = tf.random.uniform([], minval=0, maxval=N_MELS - f, dtype=tf.int32)
        mask = tf.concat([
            tf.ones([f0, TIME_FRAMES, 1]),
            tf.zeros([f, TIME_FRAMES, 1]),
            tf.ones([N_MELS - f0 - f, TIME_FRAMES, 1])
        ], axis=0)
        mel = mel * mask

    num_time_mask = 1
    for _ in range(num_time_mask):
        t = tf.random.uniform([], minval=0, maxval=25, dtype=tf.int32)
        t0 = tf.random.uniform([], minval=0, maxval=TIME_FRAMES - t, dtype=tf.int32)
        mask = tf.concat([
            tf.ones([N_MELS, t0, 1]),
            tf.zeros([N_MELS, t, 1]),
            tf.ones([N_MELS, TIME_FRAMES - t0 - t, 1])
        ], axis=1)
        mel = mel * mask
...
def se_block(x, filters, reduction=16, block_id=0):
    ...
    return layers.Multiply(name=prefix + 'mul')([x, se])

def inverted_residual_se(x, filters, kernel_size, block_id, expand_ratio=6, strides=(1, 1), se_reduction=16):
...
def build_mynanet(num_classes):
    ...
    return keras.Model(inputs, outputs, name="MynaNet_PCEN")
...
# Save with a distinct experimental name so v2 is NEVER overwritten
save_path = os.path.join(DATASET_DIR, 'best_model_mynanet_pcen.keras')
checkpoint = ModelCheckpoint(save_path, save_best_only=True, monitor="val_accuracy")

print(f"\n--- Lancement de l'Entraînement de MynaNet PCEN + Égalisation ---")
...
print(f"\n==========================================")
print(f"Meilleure Précision de Validation MynaNet PCEN: {best_val_acc * 100:.2f}%")
print(f"==========================================")
