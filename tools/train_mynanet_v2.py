...
checkpoint = ModelCheckpoint(os.path.join(DATASET_DIR, 'best_model_mynanet_v2.keras'), save_best_only=True, monitor="val_accuracy")
...
model.save(os.path.join(DATASET_DIR, 'saved_model_mynanet_v2.keras'))
...
plt.title('MynaNet v2 Training Curves')
plt.ylabel('Accuracy')
plt.xlabel('Epoch')
plt.legend()
plt.savefig(os.path.join(DATASET_DIR, 'mynanet_training_history_v2.png'))
print("MynaNet v2 model saved and curves generated.")
